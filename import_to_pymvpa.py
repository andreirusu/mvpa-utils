...
def main():
    ...
    for subject_dir in contents :
        ...
        print(len(volumes))
        ds = fmri_dataset(samples = volumes, targets = get_labels(mat), chunks = get_chunks(mat), mask=os.path.join(EXPERIMENT_DIR, subject_dir, 'roimask.nii'))
        print(ds.shape)
        print(ds.nfeatures)
        print(ds.targets)
        print(ds.chunks)
        ds.save(os.path.join(EXPERIMENT_DIR, EXPORT_DIR, CURRENT_TASK + '.'+subject_dir+'.roi.hdf5'))
