...
def main():
    ...
    for subject_dir in contents :
        ...
        ind  = [0, 64]
        experiment_offset = 6 
        valid_offset = 6 + experiment_offset
        ...
        if ds.targets[ind[current_class]] == ds.targets[ind[next_class]] or  ((sorted_onsets[i+1] - sorted_onsets[i]) > valid_offset) :
        ...
        for i in range(1, np.size(sorted_onsets)-1,1) :
            ...
            if (ds.targets[ind[current_class]] == ds.targets[ind[next_class]] or ((sorted_onsets[i+1] - sorted_onsets[i]) > valid_offset)  ) and  ( ds.targets[ind[current_class]] == ds.targets[ind[prev_class] - 1] or  ((sorted_onsets[i] - sorted_onsets[i-1]) > valid_offset) ) :
                valid[ind[current_class]] = True 
            ind[current_class] += 1
            print('Valid_count: ' + str(np.sum(np.array(valid) == True)))
        ...
        if ( ds.targets[ind[current_class]] == ds.targets[ind[prev_class] - 1] or  ((sorted_onsets[i] - sorted_onsets[i-1]) > valid_offset) ) :
            valid[ind[current_class]] = True 
        ind[current_class] += 1
        print('Valid_count: ' + str(np.sum(np.array(valid) == True)))
