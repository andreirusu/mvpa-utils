...
%         
%         task(i).all_sess.RT=[];
%         task(i).all_sess.stim_ranks=[];
%         
%        
%         
%         for k=1:nsess
%             
%             task(i).all_sess.final_xpos=[task(i).all_sess.final_xpos task(i).sess(k).final_xpos];
%             task(i).all_sess.RT=[task(i).all_sess.RT task(i).sess(k).RT];
%             task(i).all_sess.stim_ranks=[task(i).all_sess.stim_ranks; task(i).sess(k).stim_ranks];
%             
%             task(i).all_sess.total_ch_xpos=[task(i).all_sess.total_ch_xpos task(i).sess(k).total_ch_xpos];
%             task(i).all_sess.press=[task(i).all_sess.press task(i).sess(k).press];
%             
%             
%         end
%     end
...
cd(dir_start)
save('sub_stuff_CATEG_temp4.mat','all_sub')
