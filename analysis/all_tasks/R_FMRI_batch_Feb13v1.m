...
dir_start    = ['/Users/' my_name '/Documents/Reward_FMRI_2013'];
dir_base    = ['/Users/' my_name '/Documents/Reward_FMRI_2013/functional'];%'/Users/dharshan/Documents/SELF_FMRI_data/functional';
dir_behav=['/Users/' my_name '/Documents/Reward_FMRI_2013/analysis/sub_mat'];%'/Users/dharshan/Documents/SELF_FMRI_data/analysis/sub_data';

%SPECIFY WHAT THINGS TO DO:
quick_test_var=0; %1 is a test (i.e. just for subject 1). 0 for full run of all subjects 1:n

curr_sub_ind=[3]; % %CURRENT NUMBER OF SUBJECTS

analy_ind=[3]; %DEFINE EVEN FOR BATCH indices of analyses to run for FIRST LEVEL
analy_rfx_ind=[]; %DEFINE FOR BATCH for RFX analysis
...
if strcmp(task_prfx,'learn')
    load 'S_sub_stuff_v5.mat'; %has all SUBJECTS inside
    param.num_sess=3;
elseif strcmp(task_prfx,'categ')
    load sub_stuff_CATEG_v6.mat %MISSING SUBs 1 and 2 (who need onsets created)
    param.num_sess=1; %number of sessions
end
load tmp2_all_sub_onsets.mat
param.num_sess=5;


%FIRST LEVEL DESIGN MATTRIX
if run_des_mx_var==1;
    ...
    for q=1:length(analy_ind)
        ...
        for j=1:length(sub_ind)
            ...
            des_mx_file=['mk_des_mx_' task_prfx '_analy_' num2str(analy)]; %name of relevant design matrix function
            feval(des_mx_file,all_sub,param); %execute function
            
            %print to screen
            sprintf('just finished design matrix for subject %d in analysis %d',curr_sub,analy);
...
if run_estimate_var==1
    ...
    for q=1:length(analy_ind)
        ...
        for j=1:length(sub_ind)
            ...
            sprintf('just finished estimating for subject %d in analysis %d',curr_sub,analy)
...
if run_rfx_var==1
    ...
    for q=1:length(analy_rfx_ind)
        ...
        if  analy_rfx==3 || analy_rfx==222%exception
        ...
        for i=1:num_contrasts %ie LOOP FOR EACH CONTRAST (ie con image)
            ...
            for s=1:num_subs %num_subs =number of subjects
                ...
                if strcmp(task_prfx,'learn')
                    ...
                    SPM.xY.P{s}=['/Users/' my_name sprintf('/Documents/SELF_FMRI_data/functional/s%d/learn/analysis/%d/con_%04d.img',curr_sub,analy,cont)];
                        %sprintf('/Users/dharshan/Documents/SELF_FMRI_data/functional/s%d/learn/analysis/%d/con_%04d.img',curr_sub,analy,cont);
                    
                elseif strcmp(task_prfx,'categ')
                    
                    SPM.xY.P{s}=['/Users/' my_name sprintf('/Documents/SELF_FMRI_data/functional/s%d/categ/analysis/%d/con_%04d.img',curr_sub,analy,cont)];
