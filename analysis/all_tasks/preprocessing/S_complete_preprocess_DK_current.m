...
%READY TO DO CATEGORIZATION PROCESSING (i.e. don't do fieldmap, structural)
...
dir_base    = ['/Users/' my_name '/Documents/SELF_FMRI_data/functional'];
%dir_base    = '/Users/dharshan/Documents/SELF_FMRI_data/functional';%\Users\guitart\Experiments\RewPunGoN_Pharmcao'; %bse directory

task_prfx={'learn';'categ'};
spm fmri
...
%list_ex_subs=[7 15 17]; %ALSO DEFINE BELOW!!list of exception subs (i.e. with 2 fieldmaps 
%list_ex_subs=[29];%ALSO DEFINE BELOW!!list of exception subs (i.e. with 2 fieldmaps 
...
if do_struct
    ...
    for k=1:num_subs
        ...
        scanDir = [dir_base fs 's' num2str(curr_sub) fs task_prfx{2} fs 'struct'];
...
if do_fieldmap
    ...
    for k=1:num_subs
        ...
        scanDir = [dir_base fs 's' num2str(curr_sub) fs task_prfx{1} fs 'FM' fs 'iter1'];
          cd(scanDir)
        [f,d]=spm_select('List',scanDir,'.*');
        files=cellstr([repmat([scanDir '/'],size(f,1),1) f]);
        P=files;%cellstr([repmat([scanDir '/'],size(f,1),1) f]);
        ...
        scanDir = [dir_base fs 's' num2str(curr_sub) fs task_prfx{1} fs 'FM' fs 'iter2'];
          cd(scanDir)
        [f,d]=spm_select('List',scanDir,'.*');
        files=cellstr([repmat([scanDir '/'],size(f,1),1) f]);
        P=files;%cellstr([repmat([scanDir '/'],size(f,1),1) f]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%
 %clear all
 %spm fmri
 curr_sub_list=[2:30]; %list of subjects to preprocess
 
for curr_task=2
   
    fs          = filesep; %file sep
    dir_base   = ['/Users/' my_name '/Documents/SELF_FMRI_data/functional'];
   % dir_base    = '/Users/dharshan/Documents/SELF_FMRI_data/functional';%\
    sess_prfx   = 'sess';
   struct_FM_prfx='categ'; %DIRECTORY prefix where structural and FM folders are
   
   
     sub_ind_tbr_con= curr_sub_list;%[14 16:19];
    sub_ind_fieldmap= curr_sub_list;
    sub_ind_realign= curr_sub_list;
    sub_ind_coregister= curr_sub_list;
    sub_ind_segment_struct= curr_sub_list;
    sub_ind_normalise_EPI= curr_sub_list;
    sub_ind_normalise_struct= curr_sub_list;
    sub_ind_smoothing= curr_sub_list;
...
param=([]);
if strcmp(task_prfx,'learn')
    num_sessions=3;
    %%%%%%%%%%%%%%%%%%%
    
    %LIST OF WHICH FMs relate to WHICH SESSIONS
    %list_ex_subs=[7 15 17]; %list of exception subs (i.e. with 2 fieldmaps)
    %list_ex_subs=[29]; %list of exception subs (i.e. with 2 fieldmaps)
    list_ex_subs=[];
...
%     
    ...
    ex_subs=([]);
    %ex_subs(29).fm_1.ind=1:2; %i.e. for sub 1, fieldmap 1 matches to sessions 1 and 2
    %ex_subs(29).fm_2.ind=3;
    
    %%%%%%
    
elseif strcmp(task_prfx,'categ')
    num_sessions=1;
    %%%%%%%%%%%%%%%%%%%
...
if tbr_con %DO FUNCTIONAL RECONSTRUCTION
    ...
    for k=1:num_subs
        ...
        for curr_sess=1:num_sessions
            ...
            matlabbatch{1}.spm.tools.tbr.traj = {strcat('/Users/',my_name,'/Documents/spm8/toolbox/TBR/trajectories/traj_allegra_64x64_theoretical_2008-02-25_19-00.mat')};
...
if fieldmap
    ...
    for k=1:num_subs
        ...
        %structural (just for display)
        structDir = [dir_base fs 's' num2str(curr_sub) fs 'eval' fs 'struct'];
        fa   = spm_select('List', structDir, '^SMA.*img' );%
        ...
        for curr_sess = 1:num_sessions % if for all sessions seperate FMs than us -> 1:n_sess
            ...
            f   = spm_select('List', scanDir, '^fM*.*0007.img' );
            ...
            if sub_ex_var==1 %exception subject
                ...
                if ismember(curr_sess,ex_subs(curr_sub).fm_1.ind) %current session should be matched to FM pair 1
                    fmDir =[dir_base fs 's' num2str(curr_sub) fs task_prfx fs 'FM' fs 'iter1'];
                elseif ismember(curr_sess,ex_subs(curr_sub).fm_2.ind) %current session should be matched to FM pair 2
                    fmDir =[dir_base fs 's' num2str(curr_sub) fs task_prfx fs 'FM' fs 'iter2'];
                end
            else
                fmDir =[dir_base fs 's' num2str(curr_sub) fs task_prfx fs 'FM' fs 'iter1'];  %FM directory (note this will need to change for ITER2 subjects)
            end
            %%%%%%%%%%%%%%%%%%%%%
            af   = spm_select('List', fmDir, '^sMA.*' ); %
            ...
            matlabbatch{1}.spm.tools.fieldmap.presubphasemag.subj.defaults.defaultsfile = {strcat('/Users/',my_name,'/Documents/spm8/toolbox/FieldMap/pm_defaults_Allegra_eFoV.m')};%{'C:\SPM8\toolbox\FieldMap\pm_defaults_Allegra_128.m'};
...
if realign
    ...
    for k=1:num_subs
        ...
        for curr_sess = 1:num_sessions %
            ...
            f   = spm_select('List', scanDir, '^fM*.*img');
            files  = cellstr([repmat([scanDir '/'],size(f,1),1) f]);
            ...
            if sub_ex_var==1 %exception subject
                ...
                if ismember(curr_sess,ex_subs(curr_sub).fm_1.ind) %vdm file is in iter1 directory
                    vdmDir =[dir_base fs 's' num2str(curr_sub) fs task_prfx fs 'FM' fs 'iter1' fs 'vdm_sess' num2str(curr_sess)];
                elseif ismember(curr_sess,ex_subs(curr_sub).fm_2.ind) %vdm file in iter2 dir
                    vdmDir =[dir_base fs 's' num2str(curr_sub) fs task_prfx fs 'FM' fs 'iter2' fs 'vdm_sess' num2str(curr_sess)];
                end
            else
                vdmDir =[dir_base fs 's' num2str(curr_sub) fs task_prfx fs 'FM' fs 'iter1'];  %FM directory (note this will need to change for ITER2 subjects)
            end
            
            %%%%%%%%%%%%%%%%%
            
            
            fvdm5   = spm_select('List', vdmDir, '^vdm5*.*01.img'  );
...
if coregister
    ...
    for k=1:num_subs
        ...
        %structural
        structDir = [dir_base fs 's' num2str(curr_sub) fs struct_FM_prfx fs 'struct'];
        fa   = spm_select('List', structDir, '^sMA.*img' );%
        ...
        sourceDir = [dir_base fs 's' num2str(curr_sub) fs task_prfx fs sess_prfx '1'];%i.e. USE first session MEAN
        fmean  = spm_select('List', sourceDir, '^mean.*\.img$');
        ...
        for curr_sess = 1:num_sessions %
            scanDir = [dir_base fs 's' num2str(curr_sub) fs task_prfx fs sess_prfx num2str(curr_sess)];
            f   = spm_select('List', scanDir, '^ufMAO*.*img');
...
if segment_struct
    ...
    for k=1:num_subs
        ...
        matlabbatch{1}.spm.spatial.preproc.data = stFile;%{'/Users/dharshan/Documents/TINF_FMRI_data/functional/s5/eval/struct/sMA07745-0008-00001-000176-00.img,1'};
        ...
        matlabbatch{1}.spm.spatial.preproc.opts.tpm = {
            strcat('/Users/',my_name,'/Documents/spm8/tpm/grey.nii')
            strcat('/Users/',my_name,'/Documents/spm8/tpm/white.nii')
            strcat('/Users/',my_name,'/Documents/spm8/tpm/csf.nii')
            };
...
if normalise_struct
    ...
    for k=1:num_subs
        ...
        fa   = spm_select('List', structDir, '^ms.*img' );%take coregistered structural file
        anatImage=cellstr([repmat([structDir '/'],size(fa,1),1) fa]);
...
if smoothing
    ...
    for k=1:num_subs
        ...
        for curr_sess = 1:num_sessions %
            scanDir = [dir_base fs 's' num2str(curr_sub) fs task_prfx fs sess_prfx num2str(curr_sess)];
            f   = spm_select('List', scanDir, '^wufMAO*.*img');
            otherImages  = [otherImages; cellstr([repmat([scanDir '/'],size(f,1),1) f])]; %i.e. concatenate all images into one lot
