...
dir_base    = '/Users/dharshan/Documents/Reward_FMRI_2013/functional';%\Users\guitart\Experiments\RewPunGoN_Pharmcao'; %bse directory
sess_prfx='sess';
task_prfx={'reward'};
...
for curr_task=1
   ...
   struct_FM_prfx='one_back'; %DIRECTORY prefix where structural and FM folders are
    ...
    sub_ind_fieldmap=[2:21 ];
    sub_ind_realign=[2:21];
    sub_ind_coregister=[2:21];
    sub_ind_segment_struct=[2:21];
    sub_ind_normalise_EPI=[2:21];
    sub_ind_normalise_struct=[2:21];
    sub_ind_smoothing=[2:21];
...
if fieldmap
    ...
    for k=1:num_subs
        ...
        for curr_sess = 1:num_sessions % if for all sessions seperate FMs than us -> 1:n_sess
            ...
            f   = spm_select('List', scanDir, '^fM*.*000007-01.img' );
...
if coregister
    ...
    for k=1:num_subs
        ...
        fa   = spm_select('List', structDir, '^sMQ.*img' );%
        ...
        for curr_sess = 1:num_sessions %
            scanDir = [dir_base fs 's' num2str(curr_sub) fs task_prfx fs sess_prfx num2str(curr_sess)];
            f   = spm_select('List', scanDir, '^ufMQ*.*img');
...
if segment_struct
    ...
    for k=1:num_subs
        ...
        matlabbatch{1}.spm.spatial.preproc.opts.tpm = {
            '/Users/dharshan/Documents/spm8/tpm/grey.nii'
            '/Users/dharshan/Documents/spm8/tpm/white.nii'
            '/Users/dharshan/Documents/spm8/tpm/csf.nii'
            };
...
if smoothing
    ...
    for k=1:num_subs
        ...
        for curr_sess = 1:num_sessions %
            scanDir = [dir_base fs 's' num2str(curr_sub) fs task_prfx fs sess_prfx num2str(curr_sess)];
            f   = spm_select('List', scanDir, '^wufMQ*.*img');
            otherImages  = [otherImages; cellstr([repmat([scanDir '/'],size(f,1),1) f])]; %i.e. concatenate all images into one lot
