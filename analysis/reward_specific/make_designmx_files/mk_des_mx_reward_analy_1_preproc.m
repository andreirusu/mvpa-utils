...
for j=1:size(r_cue_onsets,1) 
    ...
    SPM.Sess(i).U(cond).dur         =   SPM.xY.RT; %repmat(block_dur, (length(SPM.Sess(i).U(cond).ons)),1);
...
%%%%% GET CORRECT MOTION PARAMTERS
load('../../../functional_preprocessing_data.mat')
first = Inf;
last = Inf;
for l=1:size(filelist,1)
    indices = strfind(char(filelist(l,1)), 'reward');
...
SPM.Sess(i).C.C    = [a(first:last) b(first:last) c(first:last) d(first:last) e(first:last) f(first:last) blk_eff1];          % [n x c double] covariates
